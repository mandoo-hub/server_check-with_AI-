#!/usr/bin/env python3
"""Phase 1 pure rule engine for U-001 ~ U-100.

Mapping key is only ITEM/@code (U-001 ... U-100).
No internal_id, AI, LLM, RAG, or model dependency is used.
"""
...
def boolf(fields,key): return first(fields,key,"false").lower()=="true"

def all_generic_rules(fields,c):
    details=[]; rs=[]
    for r in c.findall("./RULE"):
        ok,d=eval_rule(fields,r); rs.append(ok); details.append(d)
...
def evaluate_item(raw_fields, c):
    ...
    if na:return na,reason,[reason]
    manual,reason=manual_override(fields,c)
    if manual:return "MANUAL",reason,[reason]
    special,reason=special_override(fields,c)
    if special:return special,reason,[reason]
    ev=get_text(c,"EVALUATOR","generic"); details=[]; ok=None

    if ev=="su_restriction":
        a=find_rule(c,"pam_restriction"); b=find_rule(c,"su_file_permissions")
        ao,d1=eval_rule(fields,a); bo,d2=eval_rule(fields,b); details=[d1,d2]; ok=ao or bo
    elif ev=="fail_lock_limit":
        ds=fields.get("pam_deny_value",[]); mc=int(num(first(fields,"lock_module_count","0")) or 0)
        if ds:
            ok=all((num(v) is not None and 1<=num(v)<=10) for v in ds); details=[f"PAM deny={ds}"]
        elif mc>0:
            v=first(fields,"faillock_deny_value","NOT_SET"); n=num(v); ok=n is not None and 1<=n<=10; details=[f"faillock deny={v}"]
        else: ok=False; details=["잠금 PAM 모듈 미설정"]
    elif ev=="session_timeout":
        rr=[find_rule(c,x) for x in ("global","user_override")]; vals=[eval_rule(fields,r) for r in rr]
        ok=all(x[0] for x in vals); details=[x[1] for x in vals]
        if boolf(fields,"csh_installed"):
            r=find_rule(c,"csh"); co,cd=eval_rule(fields,r); cnt=int(num(first(fields,"csh_config_file_count","0")) or 0)
            ok=ok and co and cnt>0; details.append(f"csh files={cnt}; {cd}")
    elif ev=="password_policy":
        rules=c.findall("./RULE"); normal=[r for r in rules if not (r.get("role") or "").startswith("complexity_option_")]
        vals=[eval_rule(fields,r) for r in normal]; details=[d for _,d in vals]
        a=[r for r in rules if r.get("role")=="complexity_option_a"]; b=[r for r in rules if r.get("role")=="complexity_option_b"]
        ao=all(eval_rule(fields,r)[0] for r in a) if a else False; bo=all(eval_rule(fields,r)[0] for r in b) if b else False
        details.append(f"complexity A={ao}, B={bo}"); ok=all(x[0] for x in vals) and (ao or bo)
    elif ev=="root_remote_login":
        a=find_rule(c,"service_disabled"); b=find_rule(c,"ssh_setting"); ao,d1=eval_rule(fields,a); bo,d2=eval_rule(fields,b); details=[d1,d2]; ok=ao or bo
        ...
        ok=bool(rec) and not bad; details=[f"files={[r['path'] for r in rec]}, bad={bad}"]
    elif ev=="ftp_root_restriction":
        vals=fields.get("ftp_root_entry",[]); ok=any(splitv(v,1)=="true" for v in vals); details=[f"root control entries={vals}"]
        ...
        if not any(boolf(fields,k) for k in ("ftp_active","nfs_active")) and not fields.get("samba_guest"):
            ...
            ok=ftp_ok and not smb_bad; details=[f"ftp_ok={ftp_ok}, samba_bad={smb_bad}"]
    elif ev=="remote_access_policy":
        tel=boolf(fields,"telnet_active"); ssh=boolf(fields,"ssh_active")
        if tel: ok=False
        elif not ssh: ok=True
        else:
            ports=first(fields,"ssh_ports",""); root=first(fields,"permitrootlogin","").lower()
        ...
        if ev=="web_dir_perm":
            rec=path_records(fields,"web_directory"); bad=[r["path"] for r in rec if r["exists"] and (octal(r["mode"]) is None or octal(r["mode"])>0o755 or r["owner"] not in ("root","www-data","apache","nginx"))]
            ok=bool(rec) and not bad; details=[f"bad={bad}"]
        elif ev=="web_file_perm":
            bad=[]; rec=[]
            for r in path_records(fields,"web_config"):
                if r["exists"]: rec.append(r); mo=octal(r["mode"]); bad += [r["path"]] if mo is None or mo>0o640 else []
            for r in path_records(fields,"web_source"):
                if r["exists"]: rec.append(r); mo=octal(r["mode"]); bad += [r["path"]] if mo is None or mo>0o644 else []
            ok=bool(rec) and not bad; details=[f"bad count={len(bad)}, sample={bad[:20]}"]
        elif ev=="web_upload_limit":
            vals=[v for v in fields.get("upload_limit",[]) if v not in ("NOT_SET","FILE_NOT_FOUND")]; ok=bool(vals); details=[f"limits={vals}"]
            ...
            apache=boolf(fields,"apache_active"); nginx=boolf(fields,"nginx_active")
            ...
            vals=fields.get("symlink_setting",[]); ok=not vals; details=[f"symlink settings={vals[:20]}"]
        elif ev=="web_cgi":
            vals=fields.get("cgi_setting",[]); ok=not vals; details=[f"cgi settings={vals[:20]}"]
        elif ev=="web_listing":
            vals=fields.get("listing_setting",[]); ok=not vals; details=[f"listing settings={vals[:20]}"]
        elif ev=="web_docroot":
            ...
            vals=fields.get("default_web_artifact",[]); bad=[v for v in vals if splitv(v,1)=="true"]; ok=not bad; details=[f"existing={bad}"]
        elif ev=="web_daemon_priv":
            vals=fields.get("web_process",[]); users=[v.split(None,1)[0] for v in vals if v.strip()]
            ok=bool(users) and any(u!="root" for u in users); details=[f"process users={users}"]
    elif ev=="umask":
        ...
        ok=bool(parsed) and all(v is not None and v>=0o22 for v in parsed); details=[f"umask={vals}"]
        ...
        ok=not bad; details=[f"checked={len(rec)}, bad={bad[:20]}"]
    elif ev=="path_env":
        p=first(fields,"root_path",""); parts=p.split(":"); bad=("." in parts or "" in parts); ok=bool(p) and not bad; details=[f"PATH={p}"]
    elif ev=="ip_restrict":
        ...
        fw=first(fields,"firewall_tool","NOT_FOUND"); ok=bool(deny) or fw!="NOT_FOUND"; details=[f"deny_rules={len(deny)}, firewall={fw}"]
    elif ev=="services_permission":
        rec=path_records(fields,"target_file"); ok=bool(rec) and all(r["owner"] in ("root","bin","sys") and octal(r["mode"]) is not None and octal(r["mode"])<=0o644 for r in rec if r["exists"]); details=[str(rec)]
    elif ev=="service_patch":
        if not boolf(fields,"dns_active") and not boolf(fields,"smtp_active"): ok=True; details=["DNS/SMTP inactive"]
        ...
        ok=boolf(fields,"rsyslog_active") and any(r["exists"] for r in conf); details=[f"daemon={first(fields,'rsyslog_active')}, conf={conf}"]
    elif ev=="log_permission":
        rec=[r for r in path_records(fields,"log_path") if r["exists"]]; bad=[]
        for r in rec:
        ...
        if ev=="db_remote":
            ...
            ok=not bad; details=[f"bind={vals}, bad={bad}"]
        elif ev=="db_audit":
            vals=fields.get("db_audit_setting",[]); ok=bool([v for v in vals if v not in ("NOT_SET","FILE_NOT_FOUND")]); details=[f"audit={vals}"]
        elif ev=="db_umask":
            v=first(fields,"current_umask",""); o=octal(v[-3:] if len(v)>3 else v); ok=o is not None and o>=0o22; details=[f"umask={v}"]
            ...
            vals=fields.get("db_resource_setting",[]); ok=bool([v for v in vals if v not in ("NOT_SET","FILE_NOT_FOUND")]); details=[f"resource={vals}"]
        ...
        if ev=="was_daemon_priv":
            vals=fields.get("was_process",[]); users=[v.split(None,1)[0] for v in vals if v.strip()]; ok=bool(users) and all(u!="root" for u in users); details=[f"users={users}"]
        elif ev=="was_password_file":
            rec=path_records(fields,"was_password_file")
            if not rec:return "MANUAL","WAS 패스워드 파일 경로를 자동 식별하지 못함",[]
            bad=[r["path"] for r in rec if octal(r["mode"]) is None or octal(r["mode"])>0o640]; ok=not bad; details=[f"bad={bad}"]
        elif ev=="was_dir_perm":
            rec=path_records(fields,"was_directory")
            if not rec:return "MANUAL","WAS 홈 디렉터리 경로를 자동 식별하지 못함",[]
            bad=[r["path"] for r in rec if octal(r["mode"]) is None or octal(r["mode"])>0o755]; ok=not bad; details=[f"bad={bad}"]
        elif ev=="was_access_log":
            vals=fields.get("access_log_setting",[]); ok=bool([v for v in vals if v not in ("NOT_SET","FILE_NOT_FOUND")]); details=[f"log settings={vals}"]
...
def display_lines(item):
    out=[]
    for f in item.findall("./FIELDS/FIELD"):
        key=f.get("key",""); src=f.get("source",""); note=f.get("note",""); val=(f.text or "").strip()
        out.append(f"- {key}{' ['+src+']' if src else ''}{' ('+note+')' if note else ''}: {val}")
    return out

def build(evidence,criteria,xmlout,txtout):
    er=ET.parse(evidence).getroot(); cr=ET.parse(criteria).getroot()
    cmap={i.get("code",""):i for i in cr.findall("./ITEM")}
    root=ET.Element("ASSESSMENT_RESULT",{"version":"2.0"})
    so=ET.SubElement(root,"SYSTEM"); es=er.find("./SYSTEM")
    if es is not None:
        for ch in list(es): ET.SubElement(so,ch.tag).text=ch.text or ""
    results=ET.SubElement(root,"RESULTS"); summary=Counter(); sections=[]
    for ei in er.findall("./ITEM"):
        code=ei.get("code",""); c=cmap.get(code); name=get_text(ei,"NAME"); fields=field_map(ei)
        if c is None: result,reason,details="MANUAL","매핑되는 인증기준이 없음",[]; grade=good=bad=""
        else:
            result,reason,details=evaluate_item(fields,c); grade=get_text(c,"GRADE"); good=get_text(c,"GOOD_STANDARD"); bad=get_text(c,"BAD_STANDARD")
        summary[result]+=1
        io=ET.SubElement(results,"ITEM",{"code":code}); ET.SubElement(io,"NAME").text=name; ET.SubElement(io,"GRADE").text=grade
        st=ET.SubElement(io,"STANDARD"); ET.SubElement(st,"GOOD").text=good; ET.SubElement(st,"BAD").text=bad
        evn=ET.SubElement(io,"EVIDENCE")
        for f in ei.findall("./FIELDS/FIELD"):
            cp=ET.SubElement(evn,"FIELD",dict(f.attrib)); cp.text=f.text or ""
        en=ET.SubElement(io,"EVALUATION"); ET.SubElement(en,"RESULT").text=result; ET.SubElement(en,"REASON").text=reason
        dn=ET.SubElement(en,"DETAILS")
        for d in details: ET.SubElement(dn,"DETAIL").text=d
        sep="="*78
        sections += [sep,f"{code}. {name}",sep,"▶ 시스템 현황","",f"▶ {reason}","","-------------- < 시스템 설정 현황 > --------------",*display_lines(ei),"",f"★ {code}. 결과 : {RESULT_KOR.get(result,result)}",""]
    sn=ET.SubElement(root,"SUMMARY")
    for k in ("GOOD","VULNERABLE","MANUAL","NA"): ET.SubElement(sn,k).text=str(summary.get(k,0))
    ET.indent(root,space="    "); ET.ElementTree(root).write(xmlout,encoding="utf-8",xml_declaration=True)
